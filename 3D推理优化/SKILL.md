...
> **版本说明**：TensorRT 和 ONNX Runtime 版本更新频繁。TensorRT 最新版本为 10.x（2026年），ONNX Runtime 最新版本为 1.18+（2026年）。建议查看官方文档获取最新版本和特性。
...
> 借鉴前端质量审计（audit）思维，建立3D推理优化质量审计框架。核心理念：系统化评估 → 问题分类 → 改进建议。

### 审计维度

| 维度 | 评估内容 | 权重 |
|------|----------|------|
| **功能完整性** | 推理流程是否完整、工具是否齐全 | 30% |
| **性能表现** | 推理速度、精度、资源占用 | 25% |
| **用户体验** | 推理流程是否清晰、操作是否简便 | 20% |
...
> 借鉴前端optimize（性能优化）思维，建立3D推理优化性能优化框架。核心理念：性能分析 → 瓶颈识别 → 优化策略 → 效果验证。
...
3D推理优化策略:
  ...
  2. 计算优化:
     - 并行计算（多GPU、多线程）
     - 批处理优化（batch size、动态batch）
     - 缓存优化（结果缓存、模型缓存）
  
  3. 内存优化:
     - 内存复用（tensor复用、buffer复用）
     - 内存池（预分配、动态分配）
     - 内存压缩（稀疏存储、量化存储）
```
...
> 借鉴前端设计系统（Design System）思维，建立3D推理优化设计系统框架。核心理念：设计令牌（Design Tokens）→ 组件库（Component Library）→ 规范文档（Standards）。
...
3D推理优化设计令牌:
  ...
  2. 性能令牌:
     - 延迟目标（<10ms、<50ms、<100ms）
     - 吞吐量目标（>100FPS、>50FPS、>30FPS）
     - 资源占用目标（<1GB、<2GB、<4GB）
...
| # | 反模式 | 为什么不要做 | 正确做法 |
|---|--------|------------|----------|
| 1 | 不测试就部署 | 未测试模型可能有隐藏bug | 部署前做完整测试 |
| 2 | 忽略精度损失 | 量化可能导致精度下降 | 量化前评估精度损失 |
| 3 | 不做性能分析 | 性能问题到后期才发现 | 定期做性能分析 |
| 4 | 硬编码模型路径 | 硬编码路径换环境就挂 | 使用相对路径或配置文件 |
| 5 | 跳过版本控制 | 模型版本混乱导致问题 | 使用语义化版本控制 |
| 6 | 不做兼容性测试 | 不同硬件可能有兼容性问题 | 测试不同硬件兼容性 |
| 7 | 忽略内存优化 | 内存溢出导致崩溃 | 做内存优化和监控 |
