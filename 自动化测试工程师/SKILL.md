...
| 反模式 | 为什么是问题 | 推荐替代 |
|--------|------------|---------|
| **过度 Mock** | 测试脆弱、绑定实现、缺乏信心 | 优先用 Fake，Mock 只用于真正外部依赖 |
| **先写代码后补测试** | 失去 TDD 的设计引导价值 | 严格"没有失败测试就没有生产代码" |
| **追求 100% 覆盖率** | 高覆盖率 + 弱断言 < 中等覆盖率 + 强断言 | 关注关键路径和边界值 |
| **测试实现细节** | 重命名内部方法就导致测试失败 | 只测试可观测行为（公开接口） |
| **测试间共享状态** | 测试不可重复、难以调试 | 每个测试独立创建测试固件 |
| **Presubmit 跑慢测试** | 拖慢开发者，反馈循环断裂 | 慢测试推迟到 Postsubmit |
| **把 checking 当 testing** | 自动化覆盖率高 ≠ 测试充分 | 用 Testing/Checking 区分，补充探索性测试 |
| **测试仪式** | 为了有流程而有流程 | 每个测试必须能失败才有价值 |
