...
🔴 **CHECKPOINT · 项目诊断验证**
- 检查内容：项目是否可访问、技术栈是否识别、源码是否存在
- 通过标准：项目根目录存在、技术栈明确、源码文件存在
- 不通过处理：提示用户检查项目路径和文件
...
🔴 **CHECKPOINT · 诊断结果验证**
- 检查内容：技术栈是否明确、测试框架是否确定、源码文件是否非空
- 通过标准：技术栈和测试框架明确，源码文件列表非空
- 不通过处理：提示用户补充项目信息或检查项目结构
...
🔴 **CHECKPOINT · 代码结构清单验证**
- 检查内容：代码结构清单是否完整、函数签名是否提取、控制流是否分析
- 通过标准：每个源码文件都有完整的结构清单
- 不通过处理：提示用户检查源码文件或补充分析
...
🔴 **CHECKPOINT · 测试场景清单验证**
- 检查内容：测试场景是否完整、边界值是否覆盖、Mock策略是否明确
- 通过标准：每个函数都有完整的测试场景清单
- 不通过处理：提示用户补充测试场景或检查推导规则
...
🔴 **CHECKPOINT · 风险排序验证**
- 检查内容：风险评分是否合理、优先级排序是否正确
- 通过标准：每个函数都有风险分，排序符合风险分高低
- 不通过处理：提示用户重新评估风险或检查评分规则
...
🔴 **CHECKPOINT · 任务清单验证**
- 检查内容：任务清单是否完整、测试用例是否覆盖所有场景、Mock策略是否明确
- 通过标准：每个任务都有完整的测试用例和Mock策略
- 不通过处理：提示用户补充任务清单或检查测试场景
...
> 借鉴前端质量审计（audit）思维，建立测试质量审计框架。核心理念：系统化评估 → 问题分类 → 改进建议。
...
| # | 反模式 | 为什么不要做 | 正确做法 |
|---|--------|------------|----------|
| 1 | 跳过代码分析直接推导场景 | 没有代码分析的场景推导是空中楼阁 | 先完成Phase 1代码语义分析 |
| 2 | 猜测业务语义 | 测试工程师不猜测业务，只提取结构 | 从代码结构确定性推导 |
| 3 | 跳过检查点 | 检查点是质量保证，跳过容易出错 | 每个检查点必须验证通过 |
| 4 | 只给一个方案 | 缺乏灵活性，无法应对变化 | 必须有上中下三策 |
| 5 | 忽略边界值 | 边界值是bug高发区 | 每个比较条件生成2-3个边界值 |
| 6 | 不做风险排序 | 所有测试同等优先级会浪费资源 | 按风险分从高到低排序 |
| 7 | 任务清单不完整 | 自动化测试工程师无法执行 | 每个任务包含完整信息 |
| 8 | 忽略Mock策略 | 外部依赖会影响测试结果 | 有外部依赖时必须明确Mock策略 |
