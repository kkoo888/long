...
> **2026年更新**：LlamaIndex 持续快速发展，最新版本 v0.14.22（2026年5月）。LlamaParse 功能进一步增强，支持更多文件格式和更准确的解析。建议查看最新文档获取新特性。
...
> 借鉴前端质量审计（audit）思维，建立RAG系统质量审计框架。核心理念：系统化评估 → 问题分类 → 改进建议。

### 审计维度

| 维度 | 评估内容 | 权重 |
|------|----------|------|
| **功能完整性** | RAG流程是否完整、组件是否齐全 | 30% |
| **检索准确性** | 检索结果是否准确、相关性是否高 | 25% |
| **生成质量** | 生成答案是否准确、流畅、有用 | 20% |
...
> 借鉴前端extract（提取可重用组件）思维，建立RAG系统组件化框架。核心理念：提取可重用组件 → 建立组件库 → 标准化接口。
...
> 借鉴前端normalize（规范化设计）思维，建立RAG系统规范化框架。核心理念：定义规范 → 一致性检查 → 规范演进。
...
RAG系统规范定义:
  ...
  2. 结构规范:
     - RAG流程规范（加载→索引→存储→查询→评估）
     - 目录结构规范（data、index、query、eval）
     - 文件命名规范（小写、连字符、扩展名）
...
> 借鉴前端onboard（设计引导流程）思维，建立RAG系统用户引导框架。核心理念：新手引导 → 功能发现 → 渐进式学习。
